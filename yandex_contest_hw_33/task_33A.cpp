...
int main33A() {
    map<char, int> chars;
    using pair_type = decltype(chars)::value_type;

    string str, raw_str;

    while(getline(cin, raw_str)){
        str += raw_str;
    }
    ...
    auto pr = max_element(begin(chars), end(chars),
                          [] (const pair_type & p1, const pair_type & p2) {
        return p1.second < p2.second;
    }
    );

    int max_iter_val = pr->second;
    for (int i = 0; i < pr->second; i++) {
        for(auto [character, frequency]: chars) {
            if (i == pr->second - 1) {
                cout << "#";
            }
            else if ((frequency - i) == max_iter_val or frequency >= max_iter_val) {
                cout << "#";
            }
            else {
                cout << " ";
